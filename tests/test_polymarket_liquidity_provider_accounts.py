...
async def test_create_order_buy(mock_clob_cls):
    ...
    account.create_order(asset, **arg)

    assert account._usdc_balance == 9.97


@patch("s2cpy.model.polymarke_core.ClobClient")
async def test_create_order_no_asset(mock_clob_cls):
    """
    更新order 取消逻辑
    1. 要改变usdc_balance
    2. 发送给下游
...
async def test_create_order_exception_flow(mock_clob_cls):
    ...
    with pytest.raises(ValueError, match="asset abc, orderPriceMinTickSize 0.2 is not valid"):
        asset = MagicMock()
        asset.mini_ticker_size = 0.2
        asset.identify = "abc"
        arg = {
            "token_id": "1234567890",
            "price": 0.006,
